...
describe("SessionRegistry", () => {
  ...
  it("listForToken filters by tokenId", () => {
    const reg = new SessionRegistry();
    const r1 = reg.create({ tokenId: 7n, signer: "0xAB", scenarioId: "s", engine: mockEngine() });
    const r2 = reg.create({ tokenId: 7n, signer: "0xCD", scenarioId: "s2", engine: mockEngine() });
    const r3 = reg.create({ tokenId: 8n, signer: "0xEF", scenarioId: "s", engine: mockEngine() });
