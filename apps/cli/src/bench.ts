// apps/cli/src/bench.ts
//
// `crucible bench` — single-command MCP agent run.
// Mirrors the loop from examples/reference-agent-ts/agent.ts,
// adapted for CLI use with env-loading, validation, and watch support.
...
export async function benchCommand(opts: BenchOpts): Promise<void> {
  ...
  const client = new Client(
    { name: "crucible-cli-bench", version: "0.1.0" },
    { capabilities: {} }
  );
